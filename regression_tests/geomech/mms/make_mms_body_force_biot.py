...
# Structured grid from mms_biot_3d.in
nx = 4
ny = 4
nz = 4
...
# Hydrostatic pressure gradient from flow (rho_w * g_z).
rho_w = 1000.0
g_z = -9.80665
dp_dz = rho_w * g_z
...
for iz in range(nzv):
    ...
    for iy in range(nyv):
        ...
        for ix in range(nxv):
            ...
            body_z[idx] = (-(lam + mu) * ddiv_dz - mu * lap_f + alpha * dp_dz) / rho

            idx += 1

cell_ids = np.arange(1, nnode + 1, dtype=np.int32)

with h5py.File("mms_body_force_biot.h5", "w") as h5:
